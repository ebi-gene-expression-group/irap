...
#
# - For 'cdna', we'll set the final transcriptome (trans_abspath) to the input
#   value. Unless this gets reset (to include spikes), this will be used
#   directly for kallisto indexing etc.
# - For 'auto' we add a string that will cause iRAP to generate a transcriptome
#   by combining the gtf and genomic fasta, using irap_gtf_to_fasta. The use of
#   spike-ins will change the inputs to irap_gtf_to_fasta to contain spikes,
#   but the process will stay the same. 
...
## transcripts
# Redefine the trans_abspath to include spike-ins
override trans_abspath:=$(dir $(trans_abspath))/$(patsubst %.fasta,%.$(spikein_fasta_prefix),$(patsubst %.fa,%.fasta,$(subst .gz,,$(notdir $(trans_abspath))))).fa
...
# The way in which we produce the transcriptome differs depending on inputs and
# parameters:
#
# - 1. user supplies a cDNA file, specifies 'cdna' to user_trans, and does not
#   specify spikes. In this case the input transcriptome is used directly
#   indexing etc- i.e. no action required.
# - 2. as 1. but with spikes. In this case we must contatenate spike sequences
#   to the supplied transcriptome.
# - 3. User requests that a transcriptome be generated (user_trans=auto).
#   irap_gtf_to_fasta produces the transcriptome. Where spikes are specified
#   they are included in the inputs to irap_gtf_to_fasta, and thereby in its
#   outputs.

$(trans_abspath): $(gtf_file_abspath) $(reference_abspath)
	irap_gtf_to_fasta -g $(gtf_file_abspath) --genome $(reference_abspath) --types '$(user_trans_biotypes)' --out $@.tmp && mv $@.tmp $@

else ifdef spikein_data

$(trans_abspath): $(subst .gz,,$(cdna_file_abspath)) $(subst .gz,,$(spikein_fasta_abspath))
	cat $^ > $@.tmp && mv $@.tmp $@
endif
